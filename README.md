...
Semantic HTML is using HTML elements that describe the content, such as `<article>` and `<section>` rather than `<div>`. `<div>` tags are often better for CSS styling, since semantic elements can have unintended behavior.

2. Name two big differences between ```display: block;``` and ```display: inline;```.
Display:block pushes the following content to a new line, while display:inline acts more like text and only takes up the space it needs on the page.

3. What are the 4 areas of the box model?
Content > padding > border > margin

4. While using flexbox, what axis does the following property work on: ```align-items: center```?
The secondary axis, y-axis/vertical.

5. Explain why git is valuable to a team of developers.
Git is valuable because it manages changes that may potentially conflict with each other, allowing review and testing before team member changes are pulled into the code of the main product. It also allows you to undo complex changes and go back to a previous version.
...
Flex flow: row wrap - on the about page facts, makes it adapt easily to different size screens so you don't have to horizontal scroll.